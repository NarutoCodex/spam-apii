import requests , os , psutil , sys , jwt , pickle , json , binascii , time , urllib3 , base64 , datetime , re , socket , threading , ssl , pytz , aiohttp , uuid
...
from ffproto import decrypt_hex_json
...
async def EncryptMajorLoginManual(open_id: str, access_token: str, region_code: str = "TW", platform: int = 4):
    """
    Build MajorLogin protobuf from the provided static JSON fields,
    injecting open_id, access_token, region_code, and platform.
    """
    # Static fields from your JSON (all escaped '=' are written as '=')
    fields = {
        1: open_id,                                   # authentication
        2: access_token,                              # authentication
        3: datetime.now().strftime("%Y-%m-%d %H:%M:%S"),  # current time
        4: "free fire",
        5: platform,                                  # platform id
        7: "1.126.9",
        ...
        20: "42.111.9.117",
        21: "en",
        22: open_id,                                  # device id – using open_id as placeholder
        23: str(platform),                            # open_id_type
        24: "Handheld",
        25: "Xiaomi 220333QBI",
        26: region_code,                              # region
        29: access_token,                             # signature_md5
        30: 1,
        41: "Vi India",
        42: "4G",
        ...
        86: "OpenGLES2",
        87: 4095,
        88: platform,                                 # unknown_88 – set to platform
        90: "Delhi",
        91: "DL",
        92: 6634,
        93: "android",
        ...
        97: 1,
        99: str(platform),                            # unknown_99
        100: str(platform),                           # unknown_100
        102: "4b004346025b0f0632",
        103: 1
    }

    # Serialize to protobuf
    serialized = await CrEaTe_ProTo(fields)   # CrEaTe_ProTo expects a dict and returns bytes

    # Encrypt with the fixed key/iv (same as before)
...
async def MajorLogin(payload):
    ...
    # Create SSL context (disable verification for testing)
    ssl_context = ssl.create_default_context()
    ssl_context.check_hostname = False
    ssl_context.verify_mode = ssl.CERT_NONE

    # Prepare headers – override Content-Type for binary data
    headers = Hr.copy()                     # start with global headers
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url,
                data=payload,                # payload is already encrypted bytes
                headers=headers,
                ssl=ssl_context,
                timeout=timeout
            ) as response:
                ...
                if response.status == 200:
                    ...
                    # Log failure details
                    print(f"[MajorLogin] HTTP {response.status} – {response_body[:200]!r} ...")
                    # Optionally try to decode as UTF-8 if it's text
                    try:
                        error_text = response_body.decode('utf-8')
                        print(f"[MajorLogin] Error message: {error_text}")
                    except UnicodeDecodeError:
                        print("[MajorLogin] (binary error response)")
                    return None

    except asyncio.TimeoutError:
        print("[MajorLogin] Request timed out after 10 seconds")
        return None
    except aiohttp.ClientError as e:
        print(f"[MajorLogin] Client error: {e}")
        return None
    except ssl.SSLError as e:
        print(f"[MajorLogin] SSL error: {e}")
        return None
    except Exception as e:
        print(f"[MajorLogin] Unexpected error: {e}")
        import traceback
        traceback.print_exc()
        return None
...
#               self.insquad = None
    ...
    async def MaiiiinE(self):
        Uid , Pw = '6705270245','43D5B30A5BC79E9E1D304AF01FE3EB5815E1F7757157871691768B26E41CBFB9'
